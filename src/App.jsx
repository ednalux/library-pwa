...
function App() {
  ...
  const [boardTab, setBoardTab] = useState('NOTICE'); // 💡 탭 상태 관리 추가
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, snap => setTodayReservations(snap.docs.map(d => ({id: d.id, ...d.data()}))));
    return () => unsub();
  }, [user]);
  ...
  // ====================================================================
  // 💡 공지사항 미노출 완벽 해결 (Events 방과 Notices 방 통합!)
  // ====================================================================
  useEffect(() => {
    if (!user) return;
  ...
  return (
    <div style={{ padding: isSmallMobile ? '10px' : '20px', width: '100%', maxWidth: '1300px', margin: '0 auto', boxSizing: 'border-box', fontFamily: 'sans-serif', background: '#f8fafc', minHeight: '100vh', position: 'relative', paddingBottom: myTicket && (viewMode === 'HOME' || viewMode === 'MAP') ? '180px' : '30px' }}>
      ...
      {showIdQR && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(15, 23, 42, 0.85)', zIndex: 999999, display: 'flex', justifyContent: 'center', alignItems: 'center', padding: '20px', boxSizing: 'border-box' }}>
          <div style={{ background: '#fff', padding: '30px', borderRadius: '24px', width: '100%', maxWidth: '340px', textAlign: 'center', boxShadow: '0 20px 50px rgba(0,0,0,0.3)', position: 'relative' }}>
            <button onClick={() => setShowIdQR(false)} style={{ position: 'absolute', top: '15px', right: '15px', background: 'transparent', border: 'none', fontSize: '1.5rem', cursor: 'pointer', color: '#64748b' }}>✕</button>
            <h3 style={{ margin: '10px 0 5px 0', fontSize: '1.4rem', fontWeight: '900', color: '#0f172a' }}>모바일 출입증</h3>
            <p style={{ margin: '0 0 25px 0', fontSize: '0.9rem', color: '#64748b', fontWeight: '600' }}>게이트 스캐너에 QR을 인식해 주세요.</p>
            ...
            <div style={{ background: '#f8fafc', padding: '25px', borderRadius: '20px', border: '1px solid #e2e8f0', display: 'flex', justifyContent: 'center', marginBottom: '20px' }}>
              <QRCodeGen value={user?.email || 'student_id'} size={180} />
            </div>
      ...
      {/* 💡 [수정됨] boardTab 값을 initialTab에 정확하게 전달 */}
        ...
        <>
          {viewMode === 'HOME' && (
            <>
              ...
              <div style={{ display: 'flex', gap: '20px', marginBottom: '25px' }}>
                <div style={{ flex: 1.5, display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '15px' }}>
                  {[
                    ...
                    // 💡 [수정됨] 탭 지시값을 명확히 NOTICE와 EVENT로 분리합니다.
                    ...
                    return (
                      <div key={idx} onClick={() => {
                        if(item.mode === 'MAP') setIsBookingMode(false); 
                        // 💡 [수정됨] 클릭 시 item.tab 값을 확인해 boardTab 상태를 업데이트
                ...
                <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '15px' }}>
                  <button onClick={() => { setIsBookingMode(true); setViewMode('MAP'); }} style={{ flex: 1, padding: '20px', background: '#2563eb', color: '#fff', borderRadius: '20px', fontSize: '1.1rem', fontWeight: '900', border: 'none', cursor: 'pointer' }}>좌석 예약하러 가기</button>
              ...
              <div style={{ marginBottom: '25px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '20px' }}>
                  ...
                  <div style={{ background: '#fff', borderRadius: '24px', padding: '25px', border: '1px solid #e2e8f0', display: 'flex', flexDirection: 'column', boxShadow: '0 4px 10px rgba(0,0,0,0.02)' }}>
                    ...
                    <button onClick={() => { setActiveFloor('1층'); setIsBookingMode(false); setViewMode('MAP'); mapRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' }); }} style={{ marginTop: 'auto', width: '100%', padding: '12px', borderRadius: '12px', border: '1px solid #e2e8f0', background: '#fff', color: '#2563eb', fontWeight: '800', fontSize: '0.9rem', cursor: 'pointer', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '6px', transition: '0.2s' }}>배치도 보기 →</button>
                  </div>
                  ...
                  <div style={{ background: '#fff', borderRadius: '24px', padding: '25px', border: '1px solid #e2e8f0', display: 'flex', flexDirection: 'column', boxShadow: '0 4px 10px rgba(0,0,0,0.02)' }}>
                    ...
                    <button onClick={() => { setActiveFloor('2층'); setIsBookingMode(false); setViewMode('MAP'); mapRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' }); }} style={{ marginTop: 'auto', width: '100%', padding: '12px', borderRadius: '12px', border: '1px solid #e2e8f0', background: '#fff', color: '#2563eb', fontWeight: '800', fontSize: '0.9rem', cursor: 'pointer', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '6px', transition: '0.2s' }}>배치도 보기 →</button>
                  </div>
                  ...
                  <div style={{ background: '#fff', borderRadius: '24px', padding: '25px', border: '1px solid #e2e8f0', display: 'flex', flexDirection: 'column', boxShadow: '0 4px 10px rgba(0,0,0,0.02)' }}>
                    ...
                    <button onClick={() => { setActiveFloor('4층'); setIsBookingMode(false); setViewMode('MAP'); mapRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' }); }} style={{ marginTop: 'auto', width: '100%', padding: '12px', borderRadius: '12px', border: '1px solid #e2e8f0', background: '#fff', color: '#3b82f6', fontWeight: '800', fontSize: '0.9rem', cursor: 'pointer', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '6px', transition: '0.2s' }}>배치도 보기 →</button>
                  </div>
                </div>
              </div>
              ...
              {/* 💡 [수정됨] 이벤트 배너 컴포넌트의 더보기 클릭 시 EVENT 탭 지정 */}
          ...
          {myTicket && (
            <div style={{ position: 'fixed', bottom: '30px', left: '50%', transform: 'translateX(-50%)', width: '90%', maxWidth: '450px', background: '#fff', borderRadius: '24px', padding: '25px', boxShadow: '0 20px 40px rgba(0,0,0,0.2)', border: '2px solid #2563eb', zIndex: 100 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '15px' }}>
                <div><span style={{ background: myTicket.status === 'OCCUPIED' ? '#dcfce7' : '#fef08a', color: myTicket.status === 'OCCUPIED' ? '#16a34a' : '#ca8a04', padding: '6px 12px', borderRadius: '8px', fontSize: '0.85rem', fontWeight: '900' }}>{myTicket.status === 'OCCUPIED' ? '✅ 입실 완료' : '🎫 오늘 예약'}</span><h3 style={{ margin: '8px 0 0 0', fontSize: '1.8rem', fontWeight: '900' }}>{myTicket.seatId} 좌석</h3></div>
              ...
              <div style={{ display: 'flex', gap: '10px', marginTop: '20px' }}>
                {myTicket.status !== 'OCCUPIED' ? (<><button onClick={() => setShowSeatQR(true)} style={{ flex: 2, padding: '16px', background: '#2563eb', color: '#fff', border: 'none', borderRadius: '14px', fontWeight: '900', cursor: 'pointer' }}>QR 인증하기</button><button onClick={handleCancel} style={{ flex: 1, padding: '16px', background: '#fee2e2', color: '#dc2626', border: 'none', borderRadius: '14px', fontWeight: '900', cursor: 'pointer' }}>취소</button></>) : (<button onClick={handleCheckout} style={{ width: '100%', padding: '16px', background: '#475569', color: '#fff', border: 'none', borderRadius: '14px', fontWeight: '900', cursor: 'pointer' }}>퇴실하기</button>)}
              </div>
            </div>
          )}
        </>
